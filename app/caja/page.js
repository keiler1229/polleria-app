...
export default function CajaPage() {
  ...
  const [tipoPedido, setTipoPedido] = useState('Mesa'); // 'Mesa' o 'Llevar'
  ...
  const enviarPedidoACocina = async () => {
    ...
    if (tipoPedido === 'Llevar' && !mesa.trim()) {
      alert('Por favor, indica el nombre del cliente para llevar.');
      return;
    }
    ...
    // Aquí se distingue claramente para la cocina si es para Llevar o para Mesa
    const etiquetaTipo = tipoPedido === 'Mesa' ? `🍽️ MESA: ${mesa.toUpperCase()}` : `🛍️ PARA LLEVAR: ${mesa.toUpperCase()}`;
    const detalleConMesa = `${etiquetaTipo} -> ${detalleTexto}`;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-gray-800 border border-gray-700 p-5 rounded-2xl shadow-2xl flex flex-col justify-between">
              <div>
                ...
                {/* Selector de Tipo de Pedido: Para Comer o Para Llevar */}
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <button
                    type="button"
                    onClick={() => { setTipoPedido('Mesa'); setMesa(''); }}
                  ...
                  <button
                    type="button"
                    onClick={() => { setTipoPedido('Llevar'); setMesa(''); }}
                    className={`py-2 rounded-xl text-xs font-bold transition border ${
                      tipoPedido === 'Llevar'
                        ? 'bg-amber-500 text-gray-950 border-amber-400 shadow-md'
                        : 'bg-gray-900 text-gray-400 border-gray-700 hover:bg-gray-700'
                ...
                <div className="mb-4">
                  <label className="block text-xs font-bold text-gray-400 uppercase mb-1">
                    {tipoPedido === 'Mesa' ? 'Número de Mesa:' : 'Nombre del Cliente:'}
                  </label>
                  <input
                    type="text"
                    value={mesa}
                    onChange={(e) => setMesa(e.target.value)}
                    placeholder={tipoPedido === 'Mesa' ? 'Ej: Mesa 4' : 'Ej: Juan Pérez'}
